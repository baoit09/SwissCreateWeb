...
REDIPS.drag = (function () {
    ...
    var init,						// initialization
		...
		scrollData = {
		    width: null,	// scroll width of the window (it is usually greater then window)
		    ...
		    obj: null
		},		// scroll object (needed in autoscroll for recursive calls)
		...
		threshold = {
		    x: 0,			// initial x, y position of mouse pointer
		    y: 0,
		...
		scroll = {
		    enable: true,	// (boolean) enable/disable autoscroll function (default is true) 
		    bound: 25,			// (integer) bound width for autoscroll
		    speed: 20
		},		// (integer) scroll speed in milliseconds
		only = {
		    div: [],			// (array) DIVid -> classname, defined DIV elements can be placed only to the marked table cell with class name 'redips-only'
		    cname: 'redips-only',		// (string) class name for marked cells (default is 'redips-only') - only defined objects can be placed there
		    other: 'deny'
		},		// (string) allow / deny dropping marked objects with "redips-only" to other cells
		mark = {
		    action: 'deny',
		    cname: 'redips-mark',
		    exception: []
		...
		style = {
		    borderEnabled: 'solid',	// (string) border style for enabled elements
		    borderDisabled: 'dotted',	// (string) border style for disabled elements
		    opacityDisabled: '',		// (integer) set opacity for disabled elements
		    rowEmptyColor: 'white'
		},	// (string) color of empty row
		tableSort = true,				// (boolean) sort tables on DIV element click
		trash = {
		    className: 'redips-trash',	// (object) contains redips-trash class name and confirmation questions for delete DIV or ROW	
		    question: null,
		    questionRow: null
		},
		...
		animation = {
		    pause: 20,		// (object) animation pause (integer), step (integer) and shift (boolean)
		    step: 2,
		    shift: false
		},
		// (object)
		shift = {
		    mode: 'horizontal1',	// shift modes (horizontal1, horizontal2, vertical1, vertical2) 
		    ...
		    overflow: 'bunch'
		},	// what to do with overflowed DIV (bunch, delete, source)
		clone = {
		    keyDiv: false,		// (boolean) if true, elements could be cloned with pressed SHIFT key
		    ...
		    drop: false
		},			// (boolean) if true, then cloned element will be always dropped to the table no matter if dropped outside of the table
